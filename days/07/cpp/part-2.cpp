...
int main(int argc, char* argv[]) 
{
  ...
  std::vector<std::size_t> directory_sizes;
  std::stack<Node*> stack;
  stack.push(&fs_root);
  while (stack.size()) {
    auto next_ptr = stack.top();
    stack.pop();
    directory_sizes.push_back(next_ptr->getSize());
  ...
  /// 40M is "30M - 70M" but without unsigned-underflow issues...
  const auto need_to_free = directory_sizes.back() - 40'000'000;
  fmt::print("need to free {}\n", need_to_free);
  for (auto v: directory_sizes) {
    if (v >= need_to_free) {
      fmt::print("Smallest freeable directory size: {}\n", v);
      break;
    }
  }
