...
int main(int argc, char* argv[]) 
{
  ...
  for (auto idx = 1;; ++idx) {
    const Node a{infile}; infile.get();
    const Node b{infile}; infile.get();
    std::string line;
    std::getline(infile, line);  /// empty separation line
  ...
  fmt::print("Sum of in-order packet pairs' indices: {}\n", in_order_sum);
